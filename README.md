...
The CUPED adjustment was done via an estimated $\theta$ using historical data rather than using experiment data because re-calculating $\theta$ during each look in sequential testing will increase the false positive rate. The CUPED $\rho$ was estimated ~.75 between pre- and post-experiment sessions totaled over 4 weeks. That seems reasonable and creates a 56% decrease in variance. $\theta$ was estimated ~.9.
